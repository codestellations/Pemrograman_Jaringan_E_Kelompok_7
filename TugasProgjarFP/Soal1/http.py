...
class HttpServer:
	...
	def http_get(self,object_address,headers):
		...
		if (object_address == '/images/' or object_address == '/pdf/'):
		...
		if (os.path.isdir(thedir+object_address)):
			#get list of all files in directory
			object_address=object_address+'/'
			file_list = glob(thedir+object_address+'*')
			str_file_list = '\n'.join(file_list)
			return self.response(200,'OK',str_file_list,dict())
		else:
...
if __name__=="__main__":
	...
	# print(d)
	d = httpserver.proses('GET /images/ HTTP/1.0')
	print(d)
